...
function PANEL:Init()
	...
	local lbl = Label( '#superdof_pp.settings', SettingsPanel )
	lbl:SetContentAlignment( 8 )
	lbl:Dock( TOP )
	lbl:SetDark( true )

	self.BlurSize = vgui.Create( 'DNumSlider', SettingsPanel )
	...
	self.Distance = vgui.Create( 'DNumSlider', SettingsPanel )
	...
	SettingsPanel:SetPos( 10, 30 )
	SettingsPanel:SetSize( 300, 90 )
	SettingsPanel:DockPadding( 8, 8, 8, 8 )
	SettingsPanel:DockMargin( 0, 0, 4, 0 )
	SettingsPanel:Dock( FILL )

	local AdvSettingsPanel = vgui.Create( 'DPanel', self )

	local lbladv = Label( '#superdof_pp.adv', AdvSettingsPanel )
	lbladv:SetContentAlignment( 8 )
	lbladv:Dock( TOP )
	lbladv:SetDark( true )

	local PassesCtrl = vgui.Create( 'DNumSlider', AdvSettingsPanel )
	PassesCtrl:SetMin( 1 )
	...
	local RadialsCtrl = vgui.Create( 'DNumSlider', AdvSettingsPanel )
	...
	local ShapeCtrl = vgui.Create( 'DNumSlider', AdvSettingsPanel )
	ShapeCtrl:SetMin( 0 )
	ShapeCtrl:SetMax( 1 )
	...
	AdvSettingsPanel:SetPos( 10, 30 )
	AdvSettingsPanel:SetSize( 150, 100 )
	AdvSettingsPanel:DockPadding( 8, 8, 8, 8 )
	AdvSettingsPanel:Dock( RIGHT )

	local ActionsPanel = vgui.Create( 'DPanel', self )

	self.Render = vgui.Create( 'DButton', ActionsPanel )
	...
	self.Screenshot = vgui.Create( 'DButton', ActionsPanel )
	...
	self.ScreenshotHiRes = vgui.Create( 'DButton', ActionsPanel )
	self.ScreenshotHiRes:SetText( language.GetPhrase( 'superdof_pp.screenshot' ) .. ' (High Resolution Camera)' )
	function self.ScreenshotHiRes:DoClick()

		local g_DoF = hrc.DoF

		g_DoF.Apply = true

		g_DoF.Distance = Distance
		g_DoF.BlurSize = BlurSize
		g_DoF.Passes = Passes
		g_DoF.Steps = Steps
		g_DoF.Shape = Shape

		RunConsoleCommand( 'screenshot_high_res' )

	end
	self.ScreenshotHiRes:Dock( RIGHT )
	self.ScreenshotHiRes:SetSize( 240, 20 )
	self.ScreenshotHiRes:DockMargin( 0, 0, 8, 0 )

	local Break = vgui.Create( 'DButton', ActionsPanel )
	Break:SetText( '5' )
	local THIS = self
	...
	ActionsPanel:DockPadding( 4, 4, 4, 4 )
	ActionsPanel:DockMargin( 0, 4, 0, 0 )
	ActionsPanel:SetTall( 28 )
	ActionsPanel:MoveToBack()
